 ...
 const [nfts, setNfts] = useState([]);
  ...
  async function initializeGame(gameId){
    ...
    if (flippandoGameMasterAddress !== undefined){
        ...
        const flippandoGameMasterContract = new ethers.Contract(flippandoGameMasterAddress, FlippandoGameMaster.abi, signer);
        const game  = await flippandoGameMasterContract.initializeGame(extractedGameId);
        console.log("result returned from FlippandoGameMaster.initializeGame()  " + JSON.stringify(game, 2, null));
  ...
  async function mintTestNFT(){
    if (flippandoAddress !== undefined){
        ...
        const txResponse  = await contract.create_test_nft("dice",  {
        gasLimit: gasLimit,
        })
        .then( (result) => 
        { 
  ...
  async function mintSingleTestNFT(){
    if (flippandoAddress !== undefined){
        ...
        // values: "squareGrid", "dice", "hexagrams"
        const gasLimit = ethers.BigNumber.from('40000000');
        const txResponse  = await contract.create_single_test_nft(gameTileType)
        .then( (result) => 
        { 
            console.log('mint test txResponse ' + JSON.stringify(result))
            result.wait()
      ...
      <div className="grid flex grid-cols-5">
        ...
        <div className="col-span-3 flex flex-col items-center pt-10">
            ...
            <div className='mb-4 w-full flex justify-end pr-20'>
              <div
                ...
                <button className="text-lg font-bold gap-6 font-quantic border-transparent focus:outline-none">
                  {flipBalance} liquid / {lockedFlipBalance + flipBalance} locked $FLIP
                </button>
            ...
            <div className='flex gap-6'>
                ...
                */}
                <div>
                <a href="#" onClick={() => { createNewGame(gameLevel, 'normal') }} >
                <button className='rounded-md bg-[#98D0E9] px-3.5 py-2.5 text-lg hover:scale-110 font-semibold font-quantic text-white shadow-sm hover:bg-black focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 border-none w-[140px] focus:outline-none'>Initialize<p>new game</p> </button>
                </a>
                </div>
            </div>
            }
            {gameStatus.includes("Flippando game created") &&
                <div>
                    <a href="#" onClick={() => { initializeGame(currentGameId) }} >
                    <button className='rounded-md bg-[#98D0E9] px-3.5 py-2.5 text-lg hover:scale-110 font-semibold font-quantic text-white shadow-sm hover:bg-black focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 border-none w-[140px] focus:outline-none'>Play <p>this game</p></button>
                </a>
                </div>
            }
              ...
              <div key={index}>
                {gameTypeChoice === "squareGrid" &&
                  <a href="#" onClick={() => selectGameTileType(gameTypeChoice)}>
                    <div className={`${gameTileType === gameTypeChoice ? "flex justify-center p-4 m-4 rounded-lg shadow-lg bg-gray-200" : "flex justify-center p-4 m-4 rounded-lg bg-gray-100"}`}>
